...
- Para cada oportunidade, o payload traz **todas as propriedades da tabela de Pipeline de `docs/07`** que o radar preenche, com os valores já convertidos para os rótulos do Notion: Oportunidade, Chave de deduplicação, Identificador externo, Instituição, Tipo, Linguagem, Modalidade, País, Estado/região, Cidade, Abrangência, Cobertura, Status validado, Status operacional, Decisão ou Decisão proposta, Prioridade, Revisão humana necessária, Prazo final, Fuso do prazo, Prazo original (texto), Abertura, Data da prorrogação, Valor máximo, Moeda, PJ aceita, Exige parceiro local, Fonte oficial confirmada, Score, Confiança, Motivo principal, Maior risco, Pendências, Próxima ação, Link oficial, Link de inscrição, Regulamento, Link de descoberta, Fonte de descoberta e Última validação. Campos sem dado seguem as regras de "Campos obrigatórios na criação" de `docs/07`.
